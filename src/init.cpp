...
void Shutdown(void* parg)
{
    ...
    RenameThread("digitalrs-shutoff");
...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help"))
        {
            ...
            std::string strUsage = _("DigitalRS version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  digitalrsd [options]                     " + "\n" +
                  "  digitalrsd [options] <command> [params]  " + _("Send command to -server or digitalrsd") + "\n" +
                  "  digitalrsd [options] help                " + _("List commands") + "\n" +
                  "  digitalrsd [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "digitalrs:"))
                fCommandLine = true;
...
{
    uiInterface.ThreadSafeMessageBox(str, _("DigitalRS"), CClientUIInterface::OK | CClientUIInterface::MODAL);
    return false;
}

bool static InitWarning(const std::string &str)
{
    uiInterface.ThreadSafeMessageBox(str, _("DigitalRS"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
    return true;
}
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -?                     " + _("This help message") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: digitalrs.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: digitalrsd.pid)") + "\n" +
        ...
        "  -port=<port>           " + _("Listen for connections on <port> (default: 58786 or testnet: 51997)") + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. DigitalRS is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s.  DigitalRS is probably already running."), strDataDir.c_str()));
    ...
    if (fDaemon)
        fprintf(stdout, "DigitalRS server starting\n");
    ...
    ********************************************************* */
    ...
    if (filesystem::exists(GetDataDir() / strWalletFileName))
    {
        ...
        if (r == CDBEnv::RECOVER_OK)
        {
            ...
            uiInterface.ThreadSafeMessageBox(msg, _("DigitalRS"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        };
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        ...
        {
            strErrors << _("Error loading wallet.dat: Wallet requires newer version of DigitalRS") << "\n";
        } else
        if (nLoadWalletRet == DB_NEED_REWRITE)
        {
            strErrors << _("Wallet needed to be rewritten: restart DigitalRS to complete") << "\n";
