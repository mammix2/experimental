...
string CRPCTable::help(string strCommand) const
{
    ...
    for (map<string, const CRPCCommand*>::const_iterator mi = mapCommands.begin(); mi != mapCommands.end(); ++mi)
    {
        ...
        if (fAllAnon)
        {
            if (strMethod != "senddrstoanon"
                && strMethod != "sendanontoanon"
                && strMethod != "sendanontodrs"
...
Value stop(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "Stop DigitalRS server (and possibly override the detachdb config value).");
    // Shutdown will take long enough that the response should get back
    if (params.size() > 0)
        bitdb.SetDetach(params[0].get_bool());
    StartShutdown();
    return "DigitalRS server stopping";
}
...
{ //  name                      function                 safemd  unlocked
  //  ------------------------  -----------------------  ------  --------
    ...
    { "senddrstoanon",          &senddrstoanon,          false,  false},
    { "sendanontoanon",         &sendanontoanon,         false,  false},
    { "sendanontodrs",          &sendanontodrs,          false,  false},
...
string HTTPPost(const string& strMsg, const map<string,string>& mapRequestHeaders)
{
    ostringstream s;
    s << "POST / HTTP/1.1\r\n"
      << "User-Agent: digitalrs-json-rpc/" << FormatFullVersion() << "\r\n"
      << "Host: 127.0.0.1\r\n"
...
void ThreadRPCServer(void* parg)
{
    ...
    RenameThread("digitalrs-rpclist");
...
void ThreadRPCServer2(void* parg)
{
        ...
        (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"]))
    {
        ...
        string strWhatAmI = "To use digitalrsd";
...
void ThreadRPCServer3(void* parg)
{
    ...
    RenameThread("digitalrs-rpchand");
...
Array RPCConvertValues(const std::string &strMethod, const std::vector<std::string> &strParams)
{
    ...
    if (strMethod == "senddrstoanon"          && n > 1) ConvertTo<double>(params[1]);
    if (strMethod == "sendanontoanon"         && n > 1) ConvertTo<double>(params[1]);
    if (strMethod == "sendanontoanon"         && n > 2) ConvertTo<int>(params[2]);
    if (strMethod == "sendanontodrs"          && n > 1) ConvertTo<double>(params[1]);
    if (strMethod == "sendanontodrs"          && n > 2) ConvertTo<int>(params[2]);
