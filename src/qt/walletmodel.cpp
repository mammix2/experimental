...
WalletModel::WalletModel(CWallet *wallet, OptionsModel *optionsModel, QObject *parent) :
    ...
    cachedBalance(0), cachedDigitalRSBal(0), cachedStake(0), cachedUnconfirmedBalance(0), cachedImmatureBalance(0),
...
qint64 WalletModel::getDigitalRSBalance() const
{
    return wallet->GetDigitalRSBalance();
}
...
void WalletModel::checkBalanceChanged()
{
    ...
    qint64 newDigitalRSBal = getDigitalRSBalance();
    qint64 newStake = getStake();
    qint64 newUnconfirmedBalance = getUnconfirmedBalance();
    qint64 newImmatureBalance = getImmatureBalance();

    if (cachedBalance != newBalance
        || cachedDigitalRSBal != newDigitalRSBal
        ...
        || cachedImmatureBalance != newImmatureBalance)
    {
        ...
        cachedUnconfirmedBalance = newUnconfirmedBalance;
        cachedImmatureBalance = newImmatureBalance;
        emit balanceChanged(newBalance, newDigitalRSBal, newStake, newUnconfirmedBalance, newImmatureBalance);
    }
}
...
WalletModel::SendCoinsReturn WalletModel::sendCoinsAnon(const QList<SendCoinsRecipient> &recipients, const CCoinControl *coinControl)
{
    ...
    if (vNodes.empty())
        return SendCoinsReturn(SCR_ErrorWithMsg, 0, QString::fromStdString("DigitalRS is not connected!"));
    ...
    {
        nBalance = wallet->GetDigitalRSBalance();
        if ((nTotalOut + nTransactionFee) > nBalance)
            return SendCoinsReturn(SCR_AmountWithFeeExceedsDigitalRSBalance, nTransactionFee);
    };
...

    {
        ...
        foreach(const SendCoinsRecipient &rcp, recipients)
        {
            ...
            if (rcp.txnTypeInd == TXT_DRS_TO_DRS
                || rcp.txnTypeInd == TXT_ANON_TO_DRS)
            {
                // -- out drs
                std::string sError;
