// Copyright (c) 2014 The DigitalRS developers
...
DigitalRSGUI::DigitalRSGUI(QWidget *parent):
    QMainWindow(parent),
    bridge(new DigitalRSBridge(this)),
    clientModel(0),
    walletModel(0),
    messageModel(0),
    ...
    nWeight(0)
{
    ...
    resize(1280, 720);
    setWindowTitle(tr("DigitalRS") + " - " + tr("Client"));
#ifndef Q_OS_MAC
    qApp->setWindowIcon(QIcon(":icons/digitalrs"));
    setWindowIcon(QIcon(":icons/digitalrs"));
#else
...
#ifdef Q_OS_WIN
    QFile html("C:/digitalrs/index.html");
#else
    QFile html("/opt/digitalrs/index.html");
#endif
...
void DigitalRSGUI::pageLoaded(bool ok)
{
...
void DigitalRSGUI::addJavascriptObjects()
{
    documentFrame->addToJavaScriptWindowObject("bridge", bridge);
}

void DigitalRSGUI::urlClicked(const QUrl & link)
{
...
void DigitalRSGUI::createActions()
{
    ...
    aboutAction = new QAction(QIcon(":/icons/bitcoin"), tr("&About DigitalRS"), this);
    aboutAction->setToolTip(tr("Show information about DigitalRS"));
    aboutAction->setMenuRole(QAction::AboutRole);
    aboutQtAction = new QAction(QIcon(":/trolltech/qmessagebox/images/qtlogo-64.png"), tr("About &Qt"), this);
    aboutQtAction->setToolTip(tr("Show information about Qt"));
    aboutQtAction->setMenuRole(QAction::AboutQtRole);
    optionsAction = new QAction(QIcon(":/icons/options"), tr("&Options..."), this);
    optionsAction->setToolTip(tr("Modify configuration options for DigitalRS"));
...
void DigitalRSGUI::createMenuBar()
{
#ifdef Q_OS_MAC
...
void DigitalRSGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if (clientModel)
    {
        ...
        if (mode != NT_FULL)
        {
            ...
            setWindowTitle(tr("DigitalRS") + " - " + tr("Wallet") + ", " + sMode);
        };
...
#ifndef Q_OS_MAC
            qApp->setWindowIcon(QIcon(":icons/digitalrs_testnet"));
            setWindowIcon(QIcon(":icons/digitalrs_testnet"));
#else
            MacDockIconHandler::instance()->setIcon(QIcon(":icons/digitalrs_testnet"));
#endif
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("DigitalRS client") + QString(" ") + tr("[testnet]"));
                trayIcon->setIcon(QIcon(":/icons/digitalrs_testnet"));
...
void DigitalRSGUI::setWalletModel(WalletModel *walletModel)
{
...
void DigitalRSGUI::setMessageModel(MessageModel *messageModel)
{
...
void DigitalRSGUI::createTrayIcon()
{
    QMenu *trayIconMenu;
#ifndef Q_OS_MAC
    ...
    trayIcon->setToolTip(tr("DigitalRS client"));
    trayIcon->setIcon(QIcon(":/icons/digitalrs"));
...
#ifndef Q_OS_MAC
void DigitalRSGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void DigitalRSGUI::aboutClicked()
{
    AboutDialog dlg;
...
void DigitalRSGUI::setNumConnections(int count)
{
    ...
    connectionsIcon.setAttribute("title", tr("%n active connection(s) to DigitalRS network", "", count));
}

void DigitalRSGUI::setNumBlocks(int count, int nTotalBlocks)
{
...
void DigitalRSGUI::error(const QString &title, const QString &message, bool modal)
{
    // Report errors from network/worker thread
    if(modal)
    {
...
void DigitalRSGUI::changeEvent(QEvent *e)
{
...
void DigitalRSGUI::closeEvent(QCloseEvent *event)
{
    if(clientModel)
    {
#ifndef Q_OS_MAC // Ignored on Mac
...
void DigitalRSGUI::askFee(qint64 nFeeRequired, bool *payFee)
{
    QString strMessage =
        tr("This transaction is over the size limit.  You can still send it for a fee of %1, "
          "which goes to the nodes that process your transaction and helps to support the network.  "
          "Do you want to pay the fee?").arg(
                BitcoinUnits::formatWithUnit(BitcoinUnits::DRS, nFeeRequired));
...
void DigitalRSGUI::incomingTransaction(const QModelIndex & parent, int start, int end)
{
...
void DigitalRSGUI::incomingMessage(const QModelIndex & parent, int start, int end)
{
    if(!messageModel)
        return;
...
void DigitalRSGUI::gotoSignMessageTab(QString addr)
{
...
void DigitalRSGUI::gotoVerifyMessageTab(QString addr)
{
...
void DigitalRSGUI::optionsClicked()
{
    bridge->triggerElement("#navitems a[href=#options]", "click");
    showNormalIfMinimized();
}

void DigitalRSGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void DigitalRSGUI::dragMoveEvent(QDragMoveEvent *event)
{
    event->accept();
}

void DigitalRSGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid DigitalRS address or malformed URI parameters."));
    }

    event->acceptProposedAction();
}

void DigitalRSGUI::handleURI(QString strURI)
{
...
void DigitalRSGUI::setEncryptionStatus(int status)
{
...
void DigitalRSGUI::encryptWallet(bool status)
{
    if(!walletModel)
        return;
...
void DigitalRSGUI::backupWallet()
{
...
void DigitalRSGUI::changePassphrase()
{
...
void DigitalRSGUI::unlockWallet()
{
    if(!walletModel)
        return;
...
void DigitalRSGUI::showNormalIfMinimized(bool fToggleHidden)
{
...
void DigitalRSGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void DigitalRSGUI::updateWeight()
{
    if (!pwalletMain)
        return;
...
void DigitalRSGUI::updateStakingIcon()
{
