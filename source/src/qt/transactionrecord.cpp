...
QString TransactionRecord::getTypeLabel(const int &type)
{
    ...
    case RecvWithAddress:
        return DigitalRSGUI::tr("Received with");
    case RecvFromOther:
        return DigitalRSGUI::tr("Received from");
    case SendToAddress:
    case SendToOther:
        return DigitalRSGUI::tr("Sent to");
    case SendToSelf:
        return DigitalRSGUI::tr("Payment to yourself");
    case Generated:
        return DigitalRSGUI::tr("Mined");
    case RecvDigitalRS:
        return DigitalRSGUI::tr("Received digitalrs");
    case SendDigitalRS:
        return DigitalRSGUI::tr("Sent digitalrs");
    case Other:
        return DigitalRSGUI::tr("Other");
    default:
        return "";
    }
}
...
QString TransactionRecord::getTypeShort(const int &type)
{
    ...
    case TransactionRecord::RecvDigitalRS:
        return "input";
    case TransactionRecord::SendToAddress:
    case TransactionRecord::SendToOther:
    case TransactionRecord::SendDigitalRS:
        return "output";
    default:
        return "inout";
    }
...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet *wallet, const CWalletTx &wtx)
{
    ...
    int64_t nCredDRS, nCredDigitalRS;
    wtx.GetCredit(nCredDRS, nCredDigitalRS, true);
    int64_t nCredit = nCredDRS + nCredDigitalRS;
    ...
    {
        if (nNet > 0 && nCredDigitalRS > 0)
        {
            // -- credit
            TransactionRecord sub(hash, nTime, TransactionRecord::RecvDigitalRS, "", "", nNet, 0);
        ...
        {
            // -- debit
            TransactionRecord sub(hash, nTime, TransactionRecord::SendDigitalRS, "", "", nNet, 0);
        ...
        for (unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++)
        {
                ...
                && txout.IsAnonOutput())
            {
                ...
                if (wallet->HaveKey(ckidD))
                {
                    ...
                    sub.type = TransactionRecord::RecvDigitalRS;
    ...
    } else
    {
        ...
        BOOST_FOREACH(const CTxIn& txin, wtx.vin)
        {
                ...
                && txin.IsAnonInput())
            {
                ...
                {
                    fAllFromMe = false;
                    break; // display as send/recv digitalrs
                };
                continue;
            };
        ...
        if (fAllFromMe)
        {
            ...
            for (unsigned int nOut = 0; nOut < wtx.vout.size(); nOut++)
            {
                    ...
                    && txout.IsAnonOutput())
                {
                    ...
                    sub.type = TransactionRecord::SendDigitalRS;
                    sub.address = CBitcoinAddress(ckidD).ToString();
