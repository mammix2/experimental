...
public:
    ...
    int64_t GetDigitalRSBalance() const;
    ...
    int64_t GetDigitalRSDebit(const CTxIn& txin) const;
    int64_t GetDigitalRSCredit(const CTxOut& txout) const;
    ...
    int64_t GetCredit(const CTransaction& tx) const
    {
        ...
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
        {
            if (tx.nVersion == ANON_TXN_VERSION
                && txout.IsAnonOutput())
            {
                nCredit += GetDigitalRSCredit(txout);
            } else
    ...
    bool GetCredit(const CTransaction& tx, int64_t& nDRS, int64_t& nDigitalRS) const
    {
        nDRS = 0;
        nDigitalRS = 0;
        BOOST_FOREACH(const CTxOut& txout, tx.vout)
        {
            if (tx.nVersion == ANON_TXN_VERSION
                && txout.IsAnonOutput())
            {
                nDigitalRS += GetDigitalRSCredit(txout);
            } else
                nDRS += GetCredit(txout);
            if (!MoneyRange(nDRS)
                || !MoneyRange(nDigitalRS))
                throw std::runtime_error("CWallet::GetCredit() : value out of range");
        }
        return true;
    };
...
public:
    ...
    mutable bool fAvailableDigitalRSCreditCached;
    mutable bool fCreditSplitCached;
    mutable int64_t nDebitCached;
    mutable int64_t nCreditCached;
    mutable int64_t nAvailableCreditCached;
    mutable int64_t nChangeCached;
    mutable int64_t nAvailableDigitalRSCreditCached;
    
    mutable int64_t nCredDRSCached;
    mutable int64_t nCredDigitalRSCached;
    ...
    void Init(const CWallet* pwalletIn)
    {
        ...
        fAvailableDigitalRSCreditCached = false;
        
        nCredDRSCached = 0;
        nCredDigitalRSCached = 0;
        ...
        nAvailableDigitalRSCreditCached = 0;
        nChangeCached = 0;
        nOrderPos = -1;
    }
    ...
    bool GetCredit(int64_t& nCredDRS, int64_t& nCredDigitalRS, bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
        {
            nCredDRS = nCredDigitalRS = 0;
            return true;
        }
        ...
        if (!fUseCache || !fCreditSplitCached)
        {
            nCredDRSCached = 0;
            nCredDigitalRSCached = 0;
            pwallet->GetCredit(*this, nCredDRSCached, nCredDigitalRSCached);
            fCreditSplitCached = true;
        };
        
        nCredDRS = nCredDRSCached;
        nCredDigitalRS = nCredDigitalRSCached;
        return true;
    }
    ...
    int64_t GetAvailableDigitalRSCredit(bool fUseCache=true) const
    {
        // Must wait until coinbase is safely deep enough in the chain before valuing it
        if ((IsCoinBase() || IsCoinStake()) && GetBlocksToMaturity() > 0)
            return 0;
        
        if (fUseCache && fAvailableDigitalRSCreditCached)
            return nAvailableDigitalRSCreditCached;
        ...
        for (unsigned int i = 0; i < vout.size(); i++)
        {
            if (!IsSpent(i))
            {
                ...
                if (!MoneyRange(nCredit))
                    throw std::runtime_error("CWalletTx::GetAvailableDigitalRSCredit() : value out of range");
            };
        };
        
        nAvailableDigitalRSCreditCached = nCredit;
        fAvailableDigitalRSCreditCached = true;
        return nCredit;
    };
